...
do_exit(int error_code) {
    ...
    if (current == proc1) {
        cprintf(" do_exit: my process will exit\n");
    }
...
static int my_proc(void *arg) {
    cprintf(">> my process is running, pid = %d, name = %s\n",current->pid,get_proc_name(current));
    return 0;
}
...
proc_init(void) {
    ...
    cprintf("proc::init:: Creating my process\n");
    int pid3 = kernel_thread(my_proc,NULL,0);
    if (pid3 <= 0) {
        panic("create kernel thread my_process failed!\n");
    }
    proc1 = find_proc(pid3);
    set_proc_name(proc1,"my_proc");
    cprintf(">> Created my_proc --> pid: %d, name: %s\n",proc1->pid,proc1->name);
}
