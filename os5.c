...
{
  int i;
  pg_tbl[0] = pg_dir + 1024 * 17;
  pg_dir[256] = (int)pg_tbl[0] | PTE_P | PTE_U | PTE_W;
  for (i = 0;i < (1 << 8);i++)
    pg_tbl[0][i] = ((i << 12) + (1 << 25)) | PTE_P | PTE_W;
}
...
setup_kernel_paging()
{
 ...
 // memset(pg_mem,0,sizeof pg_mem);
  ...
  for (i = 1;i < 17;i++) pg_tbl[i] = pg_tbl[i - 1] + 1024; 
  for (i = 0;i < 17;i++)
    pg_dir[i + 768] = (int)pg_tbl[i] | PTE_P | PTE_W;
  for (i = 0;i < (1 << 14);i++)
    pg_tbl[0][i] = (i << 12) | PTE_P | PTE_W;
  pg_dir[0] = pg_dir[768];
}
