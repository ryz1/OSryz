...
{
	while (1) {
	        if (((*linkm[head]) & PTE_A) == 0) return ((*linkm[head]) >> 22);
	        *linkm[head] = ((*linkm[head]) ^ PTE_A);
	        head = (head + 1) % (UPHYSZ / PAGE);
        }
	return -1;
}
...
mappage(uint *pd, uint va, uint pa, int perm)
{
  ...
  if (*(pde = &pd[va >> 22]) & PTE_P)
    ...
    *pde = (V2P+(uint)(pt = memset(kalloc(), 0, PAGE))) | PTE_P | PTE_W | PTE_U | PTE_A;
  pte = &pt[(va >> 12) & 0x3ff];
		  ...
		  if ( va >= init->sz) {
			  ...
			  if (*ptep == 0 ) {
			     if(mem_maps_num>=UPHYSZ/PAGE) { //need swap out
				    ...
				    mappage(init->pdir, va & -PAGE, pa  & -PAGE, PTE_P | PTE_W | PTE_U | PTE_A);
			     } else {
				    printf("ADD new phy page\n");
				    mappage(init->pdir, va & -PAGE, V2P + (memset(kalloc(), 0, PAGE)), PTE_P | PTE_W | PTE_U | PTE_A);
			     }
